...
def add_log_gui(sender, channel_name, text, style_tag="normal", translated=""):
    ...
    text_widget.insert(tk.END, f"[{timestamp}] Từ {sender} trong {channel_name}\n")
    mid_idx = text_widget.index("end-1c")
    # Nội dung + bản dịch mới apply style
    text_widget.insert(tk.END, f"{text}\n{translated}\n\n")
    end_idx = text_widget.index("end-1c")
    text_widget.tag_add(style_tag, mid_idx, end_idx)
