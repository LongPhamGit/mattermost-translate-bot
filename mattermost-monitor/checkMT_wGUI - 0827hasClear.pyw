...
# Clear button (Yêu cầu: thêm nút "Clear" bên phải để xóa nội dung GUI)
def clear_display():
    # xóa nội dung hiển thị trên GUI (không ảnh hưởng file log)
    text_widget.config(state=tk.NORMAL)
    text_widget.delete("1.0", tk.END)
    text_widget.config(state=tk.DISABLED)

btn_clear = tk.Button(frame_top, text="Clear", command=clear_display)
btn_clear.pack(side=tk.RIGHT, padx=8, pady=6)
...
# Set read-only by default (user cannot edit)
text_widget.config(state=tk.DISABLED)
...
def add_log_gui(sender, channel_name, text, style_tag="normal", translated=""):
    ...
    Trước khi insert mở state -> insert -> khóa lại.
    """
    global msg_count
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    # Sau khi insert xong, khóa lại (read-only)
    text_widget.config(state=tk.DISABLED)

    # append to HTML log (không thay đổi)
