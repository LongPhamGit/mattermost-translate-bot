...
def call_gemini_translate(text: str, target_language: str = "vi") -> str:
    """
    Gọi Gemini theo mẫu bạn đưa: contents/parts + X-goog-api-key header.
    Lưu ý: GEMINI_URL & API_KEY phải đúng endpoint/key bạn được cấp.
    """
...
def init_html_log():
    if not os.path.exists(HTML_LOG_FILE):
        with open(HTML_LOG_FILE, "w", encoding="utf-8") as f:
            ...
            f.write(".ch_mention {color:#1a73e8; font-weight:bold;}")
...
btn_bring = tk.Button(frame_top, text="Hiện cửa sổ (Bring to front)", command=bring_to_front)
btn_bring.pack(side=tk.LEFT, padx=8, pady=6)
...
# Tag styles
text_widget.tag_config("mention", foreground="#b00020")   # red-ish
text_widget.tag_config("ch_mention", foreground="#1a73e8")# blue
...
def notify_with_click(title: str, message: str, on_click=None):
    """
    - Windows + win10toast-click: hỗ trợ click để gọi on_click (đưa GUI lên).
    - Khác: fallback plyer (không có click).
    """
...
def detect_mentions(text: str):
    """
    Trả về tuple: (is_personal_mention, is_channel_mention)
    """
    t = text.lower()
...
def on_message(ws, message):
    ...
    if data.get("event") == "posted":
        ...
        # Chỉ xử lý trong các channel theo dõi
        ...
        # Popup notification khi có mention cá nhân hoặc mention toàn kênh
